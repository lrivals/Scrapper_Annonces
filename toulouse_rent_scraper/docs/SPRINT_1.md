...
**Statut** : 🔵 En cours — 3/6 tâches terminées, 3 reportées (notifications)
...
**Résultat** : `MIN_RENT_EUR = 50` ajouté dans `config.py`, filtre appliqué dans `apply_price_and_distance_filter()`. 24 annonces aberrantes (11-21€) purgées de la DB. 4 tests ajoutés.
...
**Résultat** : `reporting/exporter.py` créé avec `export_csv()` et `export_json()`. Option CLI `--export csv/json`. 10 tests unitaires.

---

### 1.3 Notifications Telegram (Priorité: Haute | Effort: M) ⏸️ Reporté
...
### 1.4 Notification email SMTP (Priorité: Moyenne | Effort: S) ⏸️ Reporté
...
**Résultat** : `storage/cleaner.py` créé. Migration auto colonne `status`. Option CLI `--purge`. Rapports et exports filtrent les expirées. 6 tests unitaires.

---

### 1.6 Tests notifications (Priorité: Moyenne | Effort: S) ⏸️ Reporté
...
| # | Tâche | Priorité | Effort | Dépendance | Statut |
|---|-------|----------|--------|------------|--------|
| 1.1 | Fix parsing prix SeLoger | Haute | S | — | ✅ |
| 1.2 | Export CSV/JSON | Haute | S | — | ✅ |
| 1.3 | Notifications Telegram | Haute | M | .env config | ⏸️ |
| 1.4 | Notification email SMTP | Moyenne | S | — | ⏸️ |
| 1.5 | Purge annonces expirées | Moyenne | M | — | ✅ |
| 1.6 | Tests notifications | Moyenne | S | 1.2, 1.3, 1.4, 1.5 | ⏸️ |
